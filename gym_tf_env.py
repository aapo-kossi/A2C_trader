...
class TradingEnv:
    def __init__(self,
        ...
        length_ranges = - tf.reverse(
            tf.repeat(tf.expand_dims(tf.range(0, self.input_days, dtype=tf.int64), 0), self.num_envs, 0), [1])
        ...
        init_ohlcvd = tf.zeros(
            (n_envs,) + train_windows.element_spec[0].shape[:-1] + (train_windows.element_spec[0].shape[-1] - 1,),
            dtype=tf.float32)
    ...
    def _reset(self, dones):
        ...
        # new_ohlcvd, new_conames, new_secs = tf.map_fn(lambda i: next(self.window), index, parallel_iterations=64,
        #       fn_output_signature=(tf.float32, tf.string, tf.float32))
        ...
        new_secs = tf.one_hot(sec_index, self.n_secs, axis=1, dtype=tf.float32)[:, 1:,
                   :]  # drop the first column as it is always empty (corresponds to GICS sector 5, which doesn't exist)
    ...
    def step(self, action):
        ...
        self.capital.assign_add(tf.reduce_sum(- a * lasts + e * divs, axis=1, keepdims=True) - commissions,
                                read_value=False)
